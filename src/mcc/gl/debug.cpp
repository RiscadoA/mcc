...
void mcc::gl::Debug::init() {
    ...
    Debug::color_loc = Debug::shader.get_uniform_location("color").unwrap();
    Debug::vp_loc = Debug::shader.get_uniform_location("vp").unwrap();
    Debug::model_loc = Debug::shader.get_uniform_location("model").unwrap();
    ...
    Debug::vb = VertexBuffer::create(sizeof(glm::vec3) * data.size(), data.data(), Usage::Static).unwrap();
    Debug::va = VertexArray::create({
         mcc::gl::Attribute(
            Debug::vb,
            sizeof(float) * 3, 0,
...
void mcc::gl::Debug::flush(const glm::mat4& vp, float dt) {
    if (!Debug::initialized) {
        return;
    }
...
void mcc::gl::Debug::draw_box(glm::vec3 position, glm::vec3 scale, glm::vec4 color, float time) {
    if (!Debug::initialized) {
        return;
    }
...
void mcc::gl::Debug::draw_sphere(glm::vec3 position, float radius, glm::vec4 color, float time) {
    if (!Debug::initialized) {
        return;
    }
...
void mcc::gl::Debug::add_command(const glm::mat4& transform, glm::vec4 color, unsigned int first, unsigned int count, float time) {
    Debug::command_mutex.lock();
